...
pub async fn get_danmu_for_page(page: biliapi::requests::VideoPage) -> Result<Vec<DanmakuElem>> {
    ...
    let s = page.duration.as_secs();
    info!("获取视频 aid={} 的弹幕，视频有 {} 秒", page.cid, s);

    let mut fut = vec![];
    for i in 0..div_ceil(s, 360) {
        fut.push(get_danmu_for_cid_segment(client.clone(), page.cid, i + 1));
    }
    let mut results =
